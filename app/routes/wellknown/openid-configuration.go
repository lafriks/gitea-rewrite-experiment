...
// OpenIDConfiguration returns OpenID configuration
func (r *Route) OpenIDConfiguration(c *gin.Context) {
	baseURL := "http://localhost:3000"
	userRegistrationURL := ""
	subjectTypes := make([]string, 0)
	claimsSupported := make([]string, 0)
	scopesSupported := make([]string, 0)
	userInfoEndpoint := baseURL + "/user/info"
	c.JSON(http.StatusOK, &WellKnown{
		Issuer:                            baseURL + "/",
		AuthURL:                           baseURL + "/oauth2/auth",
		TokenURL:                          baseURL + "/oauth2/token",
		JWKsURI:                           baseURL + "/.well-known/jwks.json",
		RegistrationEndpoint:              userRegistrationURL,
